...
def _make_pvalues(per_table_data) -> CollectedPvalues:
...
class TestComputeKey:
    ...
    def test_different_csv_bytes_different_key(self, tmp_path):
        ...
        (sub / "collapsed_pvalues.csv").write_text(
            "gene_id,pvalue\n1,0.05\n"
        )
        for d in (full, sub):
            (d / "raw_pvalues.csv").write_text("gene_id,pvalue\n1,0.05\n")
    ...
    def test_different_script_bytes_different_key(self, tmp_path):
    ...
    def test_no_collision_under_concatenation(self, tmp_path):
...
class TestStoreAndLookup:
    def test_round_trip(self, isolated_cache, tmp_path):
    ...
    def test_store_is_atomic(self, isolated_cache, tmp_path):
...
class TestCallRCombineCaching:
    ...
    def test_second_call_hits_cache(self, isolated_cache):
    ...
    def test_no_cache_flag_bypasses_cache(self, isolated_cache):
        pvalues = self._pvalues()
    ...
    def test_test_mode_subset_does_not_collide(self, isolated_cache):
