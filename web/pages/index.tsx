...
export default function Home() {
  ...
  const resolveSymbol = async (
    ...
    // CONTROL is a pseudo-symbol; resolve it directly to the sentinel
    // suggestion without round-tripping the API. The dropdown count is a
    // cosmetic placeholder here — autocomplete reports the precise count.
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const hydrate = async () => {
      // Compute both next-state values *before* applying either. If we
      // setPerturbed(null) synchronously and then `await resolveSymbol`
      // for the target, the URL-sync effect fires in between with
      // `perturbed=null, target=null`, replaces the URL with `/`, and
      // the resulting query change re-triggers this hydration with
      // cancelled=true — orphaning the in-flight target resolve. Resolve
      // both first, then apply both atomically so React can batch.
      let nextPerturbed: SearchSuggestion | null | undefined = undefined;
      let nextTarget: SearchSuggestion | null | undefined = undefined;

      if (qPerturbed) {
        if (perturbed?.humanSymbol !== qPerturbed) {
          const sp = await resolveSymbol(qPerturbed);
          if (cancelled) return;
          if (sp) nextPerturbed = sp;
        }
      } else if (perturbed) {
        nextPerturbed = null;
      }
      ...
      if (nextPerturbed !== undefined) setPerturbed(nextPerturbed);
  ...
  const simpleGeneString = (gene: SearchSuggestion | null) => {
    if (!gene) return "Any";
    // The "all controls" sentinel is a synthetic set, not a species-tagged
    // gene; render plainly without the "(human)" suffix.
    if (gene.kind === "control" && gene.humanSymbol === "CONTROL") {
      return "CONTROL (all controls)";
    }
          ...
          >
            <div>
              ...
              >
                ...
                <InfoTooltip
                  size={13}
                  text="The gene that was experimentally manipulated in this dataset's experiment — knocked down (CRISPRi, RNAi/shRNA), upregulated (CRISPRa, overexpression), knocked out (CRISPR-KO), or carried as a mutant allele. Leave blank to skip the perturbation filter. Type CONTROL to match all control genes (e.g. NonTarget1, SafeTarget) across all datasets."
                />
              </div>
              <SearchBar
            ...
            <div>
              ...
              >
                ...
                <InfoTooltip
                  size={13}
                  text="The gene whose expression or activity was measured as a readout — i.e. how the genome responded. Leave blank to skip the readout filter. Type CONTROL to match all control genes (e.g. NonTarget1, SafeTarget) across all datasets."
                />
              </div>
              <SearchBar
