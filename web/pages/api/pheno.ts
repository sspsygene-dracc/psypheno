...
function queryForPage(page?: string | string[] | undefined) {
  switch (page) {
    case "deg":
      return {
        sql: "select hgnc.symbol, m.gene, m.cellType, m.logFC, m.PValue, m.perturbation, m.target from mousePerturb4Tf as m, hgnc where hgnc.hgnc_id=m.hgnc_id order by perturbation;",
        fields: [
          "Perturbed Gene",
          "Mouse Effect Gene",
          "Cell Type",
    ...
    case "sizes":
      return {
        sql: "select symbol, Forebrain, Optic_Tectum, Thalamus, Hypothalamus, Cerebellum, Hindbrain, Habenula, Posterior_Tuberculum,Mutant_Experiment_Sample from zebraAsdSizes as z, hgnc where hgnc.hgnc_id=z.hgnc_id;",
        fields: [
          "KO gene (Human)",
          "Foreb Size",
          "Optic Tec Size",
    ...
    case "perturbFishAstr":
      return {
        sql: "SELECT symbol, gene, LFC, qVal from perturbFishAstro as p, hgnc where hgnc.hgnc_id=p.hgnc_id;",
...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (format === "tsv") {
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="geneTable.tsv"'
    );
