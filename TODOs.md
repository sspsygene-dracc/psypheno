...
- during data loading, if R is not available, print a bold, amber warning that we can't compute the combined p-values data, and leave them blank in the DB, rather than erroring. If R is installed but packages are missing, attempt to install them during load-db rather than warning or erroring immediately. Only if the the packages can't be installed, print the warning and skip p-value combinations. The rest of the data on the p-value combinations page (individual tables) should still be displayed.
