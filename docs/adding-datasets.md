...
> **`sspsygene push-data <name> --instance dev`** (see
> [7c](#7c-push-your-data-files-with-sspsygene-push-data) below), or pass
...
[7c](#7c-push-your-data-files-with-sspsygene-push-data) for how to push them).

### 7c. Push your data files with `sspsygene push-data`
...
`sspsygene push-data` does that push from your laptop. The full flow:
...
#    overwrite them:
sspsygene push-data my-dataset --instance dev

# 3. Deploy + rebuild the dev DB (7a):
sspsygene deploy --instances dev --load-db
```

`push-data` takes one or more dataset names (there is no implicit "all"),
plus `--instance dev|int|prod` (default `dev`), `--host` (default `hgwdev`),
and `--dry-run`:

```bash
sspsygene push-data sfari psychscreen --instance int
sspsygene push-data my-dataset --instance dev --dry-run   # preview only
```
